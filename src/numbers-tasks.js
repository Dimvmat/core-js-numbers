...
function getCircleCircumference(radius) {
  const circle = 2 * 3.141592653589793 * radius;
  return circle;
}
...
function roundToPowerOfTen(num, pow) {
  ...
  if (pow === 0) {
    ...
    result += Math.round(num * 0.1) * 10;
  } else if (pow === 2) {
    result += Math.round(num * 0.01) * 100;
  } else {
    result += Math.round(num * 0.001) * 1000;
  }
  return result;
}
...
function getSumOfNumbers(x1, x2, x3) {
  return Number((x1 + x2 + x3).toFixed(2));
}
